...
Scam/Organiser structure changed
	!There is now a powershell script included to create a copy of SCM_TemplateClean, wich changes the names as well >> see readme + script in /_SCM!
	...
	In your scm_xxxx block:
		eS:myEnum		is now gone
		Step:utl_Step	is now gone
		State:SCM_State is the new state + handler
		-rename eS to State
		-change type to /EnumsAndState/sXXXX  
			>> autogenerated if you used the script to copy the template.
			>> when upgrading an old project:
				-copy and rename sTpc from the TemplateClean
				-change eS:eTpc in the input declaration to your enum
				-done
		-The State block needs to be called once before the state - transitions >> template location = top in A_Exceptions
		-A step transition is done by calling State.M_Step(eMyStateEnum.NewState) !Note this is the minimal version, you can add more arguments > see template!
		-State.P_StepTime >> handy property that shows elapsed time for actual step (can be used for example for simulation purposes)

General changes
	>> <V0.7 code-blocks split into Actions: exceptions / simulation / transitions / activations
